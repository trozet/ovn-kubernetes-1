...
func createPerNodeVIPs(svcIPs []string, protocol v1.Protocol, sourcePort int32, targetIPs []string, targetPort int32) error {
	...
	for _, gatewayRouter := range gatewayRouters {
		...
		// if empty svcIP is provided, then physical IPs will be used for the VIP
		vips := physicalIPs
		if len(svcIPs) > 0 {
			vips = svcIPs
		}
		...
		// 'load_balancer'.
		err = loadbalancer.CreateLoadBalancerVIPs(gatewayLB, vips, sourcePort, newTargets, targetPort)
		if err != nil {
		...
		if config.Gateway.Mode == config.GatewayModeShared {
			...
			err = loadbalancer.CreateLoadBalancerVIPs(workerLB, vips, sourcePort, targetIPs, targetPort)
			if err != nil {
...
// getNodeIPs returns the IPs for every node in the cluster
